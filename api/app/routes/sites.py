...
def calculate_site_revenue_from_shares(site, db: Session):
    ...
    Utilise la formule CRC = C × S/D (Coinbase Reward × Shares / Difficulty)
...
async def calculate_site_profit(
    ...
    """Calculer le profit total d'un site avec toutes ses machines en utilisant les accepted shares"""
    # Vérifier que le site existe
    site = db.query(models.MiningSite).filter(models.MiningSite.id == site_id).first()
    if not site:
        raise HTTPException(status_code=404, detail="Site non trouvé")
    ...
    for site_machine in site_machines:
        ...
        if template:
            # Le revenu est maintenant calculé pour le site entier, pas par machine.
            # On se concentre ici sur le calcul des coûts.
            # TODO: Utiliser les vraies fonctions SQL avec paliers pour le coût
